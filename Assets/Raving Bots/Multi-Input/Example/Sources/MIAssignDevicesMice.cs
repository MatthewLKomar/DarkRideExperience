...
// and updating the UI
public class MIAssignDevicesMice : MonoBehaviour {
    ...
    private void AssignDevices() {
        ...
        foreach (var device in _inputState.Devices) {
            ...
            //print(device.SupportedAxes.Contains("MouseRight"));
            print(DeviceOfType("Mouse", device));
            if (device.Name.Contains("Spinner") && p < numPlayers) {
                if (!IsAssigned(device)) {
                    _players[p].Assign(device);
                    UpdateDeviceList();
                    p += 1;
                }
            }
